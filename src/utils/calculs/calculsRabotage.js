// ─── CALCULS RABOTAGE ────────────────────────────────────────────────────────
// Calculs spécifiques aux chantiers de rabotage (évacuation fraisat)
// Cycle inversé : chantier → chargement fraisat → centrale → déchargement → chantier
  ...
  for (const centrale of centrales) {
    ...
    options.push({
      ...
      distanceKm:    Math.round(Math.sqrt(
        Math.pow((chantier.lat - centrale.lat) * 111, 2) +
        Math.pow((chantier.lng - centrale.lng) * 78, 2)
      )),
      coutTotal:     Math.round(coutTotal),
      coutTonneNet,
      revenuFraisat: Math.round(revenuFraisat),
...
export function calculerRotationsRabotage(chantier) {
  ...
  const tempsCycle =
    type.temps_sur_chantier_rabotage +
    tempsTrajet +
    type.temps_dechargement_rabotage +
    tempsTrajet;
  ...
  console.log("=== CALCUL ROTATIONS RABOTAGE ===");
  ...
  console.log("entierInf:", entierInf);
  console.log("Proche centrale (< 15 min):", tempsTrajet < 15);
  console.log("Rotations illimitées:", chantier.rotationsIllimitees);
  console.log("→ rotationsParCamionFinal:", rotationsParCamionFinal);
  console.log("Rotations totales:", rotationsTotales);
  console.log("→ Nb camions:", nbCamions);
  console.log("=================================");

  return {
    chantierNom:         chantier.nomChantier,
    typeChantier:        "fraisat",
    centraleId,
    tonnage,
    capacite,
...
// ─── GÉNÉRATION PLANNING CAMION RABOTAGE ─────────────────────────────────────
export function genererPlanningCamionRabotage(camion, chantier, calc, decalage = 0) {
  const rotations = [];
  const type = getTypeCamion(chantier.typeCamion);

  let cursor = calc.heureDebutChantier + decalage * type.temps_sur_chantier_rabotage;
